...
| **Quick select** `F5` | click fills every connected matching tile — by colour, or by **Tile type** (its tiledata name), so one click takes a whole cave. Drag to limit the radius |
